...
export default defineEventHandler(async (event) => {
  ...
  if (!image.type || !ALLOWED_TYPES.has(image.type)) {
    throw createError({ statusCode: 400, message: 'Поддерживаются только JPG/PNG/WEBP' })
  }

  if (image.data.length > MAX_IMAGE_BYTES) {
    throw createError({ statusCode: 400, message: 'Размер файла не должен превышать 8 МБ' })
  }
